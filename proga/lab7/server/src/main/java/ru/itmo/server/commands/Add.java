    ...
    public Response execute(Request request) {
        try {
            ...
            if (!userManager.checkUser(userManager.getUser(UserManager.getCurrentUser())) || UserManager.getCurrentUser() == 0) {
                return new AddResponse(-1, "Вы не вошли. Используйте команду Authorization");
            } else if (!req.getPerson().verificate()) {
                return new AddResponse(-1, "Человек не добавлен, так как поля не валидны!");
            } else {
                Long nextId = databaseManger.addPerson(req.getPerson());

                if (nextId == null) {
                    return new AddResponse(-1, "Ошибка при добавлении человека в базу данных.");
                } else {
