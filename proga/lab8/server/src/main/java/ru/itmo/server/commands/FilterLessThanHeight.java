    ...
    public Response execute(Request request) {
        try {
            if (!userManager.checkUser(userManager.getUser(UserManager.getCurrentUser()))) {
                return new FilterLessThanHeightResponse(null, "Вы не вошли. Используйте команду Authorization", 1);
            } else {
                var req = (FilterLessThanHeightRequest) request;
                return new FilterLessThanHeightResponse(filteredByHeight(req.getHeight()), null, 0);
            }
        } catch (Exception e) {
            return new FilterLessThanHeightResponse(null, e.toString(), 2);
        }
    }

    private List<Person> filteredByHeight(double height) {
        return collectionVault.getCollection().stream().filter(person -> (person.getHeight() < height)).sorted(new PersonComparator()).collect(Collectors.toList());
    }
}