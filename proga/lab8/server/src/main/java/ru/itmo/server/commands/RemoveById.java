    ...
    public Response execute(Request request) {
        try {
            ...
            if (!collectionVault.existId(req.getId())) {
                return new RemoveByIdResponse("Человека с таким ID не найдено", 1);
            } if (UserManager.getCurrentUser() != collectionVault.getById(req.getId()).getCreator()) {
                return new RemoveByIdResponse("Человек с ID " + req.getId() + " создан другим пользователем", 2);
            } else if (!userManager.checkUser(userManager.getUser(UserManager.getCurrentUser()))) {
                return new RemoveByIdResponse("Вы не вошли. Используйте команду Authorization", 3);
            } else {
                ...
                return new RemoveByIdResponse(null, 0);
            }
        } catch (Exception e) {
            return new RemoveByIdResponse(e.toString(), 4);
        }
    }
}
