    ...
    public Response execute(Request request) {
        try {
            if (!userManager.checkUser(userManager.getUser(UserManager.getCurrentUser()))) {
                return new CountGreaterThanWeightResponse(null, "Вы не вошли. Используйте команду Authorization", 1);
            } else {
                var req = (CountGreaterThanWeightRequest) request;
                return new CountGreaterThanWeightResponse(counter(req.getWeight()), null, 0);
            }
        } catch (Exception e) {
            return new CountGreaterThanWeightResponse(null, e.toString(), 2);
        }
    }
