    ...
    public Response execute(Request request) {
        try {
            ...
            if (!userManager.checkUser(userManager.getUser(UserManager.getCurrentUser()))) {
                return new RemoveLowerResponse(null, "Вы не вошли. Используйте команду Authorization", 1);
            } else {
                databaseManger.removeLower(req.getHeight(), UserManager.getCurrentUser());

                return new RemoveLowerResponse(remove(req.getHeight()), null, 0);
            }
        } catch (Exception e) {
            return new RemoveLowerResponse(null, e.getMessage(), 2);
        }
    }
