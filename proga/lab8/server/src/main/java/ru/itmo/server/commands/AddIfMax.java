        ...
        try {
            ...
            if (req.getPerson().getHeight() > maxHeight) {
                if (!req.getPerson().verificate()) {
                    return new AddIfMaxResponse(-1, false, "Человек не добавлен, так как поля не валидны!", 1);
                } else if (!userManager.checkUser(userManager.getUser(UserManager.getCurrentUser()))) {
                    return new AddIfMaxResponse(-1, false, "Вы не вошли. Используйте команду Authorization", 2);
                } else {
                    ...
                    return new AddIfMaxResponse(nextId, true, null, 0);
                }
            } else {
                return new AddIfMaxResponse(-1, false, "Человек не добавлен, так как его рост не является максимальным", 3);
            }
        } catch (Exception e) {
            return new AddIfMaxResponse(-1, false, e.toString(), 4);
        }
    }
