    ...
    public Response execute(Request request) {
        try {
            ...
            if (!userManager.checkUser(userManager.getUser(UserManager.getCurrentUser()))) {
                return new UpdateIdResponse("Вы не вошли. Используйте команду Authorization", 4);
            }
            if (!collectionVault.existId(req.getId())) {
                return new UpdateIdResponse("Человека с таким ID не существует", 1);
            }
            if (UserManager.getCurrentUser() != collectionVault.getById(req.getId()).getCreator()) {
                return new UpdateIdResponse("Человек с ID " + req.getId() + " создан другим пользователем", 2);
            }
            if (!req.getUpdatedPerson().verificate()) {
                return new UpdateIdResponse("Человек не обновлен, так как поля не валидны!", 3);
            }
            databaseManger.updatePerson(req.getUpdatedPerson());

            collectionVault.getById(req.getId()).update(req.getUpdatedPerson());
            return new UpdateIdResponse(null, 0);
        } catch (Exception e) {
            return new UpdateIdResponse(e.toString(), 5);
        }
    }
}
