    ...
    public Response execute(Request request) {
        try {
            ...
            if (!userManager.checkUser(req.getLogin())) {
                return new AuthorizationResponse(null, "Пользователь с логином " + req.getLogin() + " не найден", 1);
            } else if (!userManager.checkPassword(req.getLogin(), req.getPassword())) {
                return new AuthorizationResponse(null, "Введен неверный пароль", 2);
            } else {
                int id = userManager.getUser(req.getLogin());
                UserManager.setCurrentUser(id);
                return new AuthorizationResponse("Выполнена авторизация!", null, 0);
            }
        } catch (SQLException | NoSuchAlgorithmException e) {
            return new AuthorizationResponse(null, "Произошла ошибка при работе с базой данных", 3);
        } catch (Exception e) {
            return new AuthorizationResponse(null, e.toString(), 4);
        }
    }
}
