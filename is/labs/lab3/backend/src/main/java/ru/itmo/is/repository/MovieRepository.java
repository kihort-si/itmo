...
import ru.itmo.is.interceptor.CacheStatisticsLogging;
...
public class MovieRepository {
    ...
    public Movie save(Movie movie) {
        ...
        if (director != null) {
            if (director.getId() == null) {
                ...
                Person managedDirector = entityManager.find(Person.class, director.getId());
        ...
        if (screenwriter != null) {
            if (screenwriter.getId() == null) {
                if (!entityManager.contains(screenwriter)) {
                    entityManager.persist(screenwriter);
                }
            } else {
                Person managedScreenwriter = entityManager.find(Person.class, screenwriter.getId());
                if (managedScreenwriter == null) {
                    throw new IllegalArgumentException("Сценарист с ID " + screenwriter.getId() + " не найден");
                }
                screenwriter = managedScreenwriter;
                movie.setScreenwriter(screenwriter);
            }
        }
        if (operator != null) {
            if (operator.getId() == null) {
                if (!entityManager.contains(operator)) {
                    entityManager.persist(operator);
                }
            } else {
                Person managedOperator = entityManager.find(Person.class, operator.getId());
                if (managedOperator == null) {
                    throw new IllegalArgumentException("Оператор с ID " + operator.getId() + " не найден");
                }
        ...
        try {
            if (movie.getId() < 0 || movie.getId() == 0) {
                ...
                if (existingMovie != null) {
                    ...
                    existingMovie.setScreenwriter(screenwriter);
                    existingMovie.setLength(movie.getLength());
                    existingMovie.setDirector(director);
                    existingMovie.setOperator(operator);
    ...
    @CacheStatisticsLogging(enabled = true)
    ...
    public Movie findByNameAndStaff(String name, Person director, Person screenwriter, Person operator) {
        ...
        cq.where(expr);
        TypedQuery<Movie> query = entityManager.createQuery(cq);
        query.setFlushMode(jakarta.persistence.FlushModeType.COMMIT);
        List<Movie> results = query.getResultList();
