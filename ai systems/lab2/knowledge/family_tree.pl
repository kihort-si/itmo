...
% Кандидаты для брака (мужчины): живы, старше 18, не в браке
marriage_candidate_male(Person) :-
    male(Person),
    age(Person, Age),
    Age >= 18,
    \+ death(_, Person),
    \+ current_married(Person, _).

% Кандидаты для брака (женщины): живы, старше 18, не в браке
marriage_candidate_female(Person) :-
    female(Person),
    age(Person, Age),
    Age >= 18,
    \+ death(_, Person),
...
% Пары, подходящие для усыновления: в браке более 3 лет, оба старше 25
adoption_candidate(X, Y) :-
    current_married(X, Y),
    marriage_duration(X, Y, Duration),
    Duration > 3,
    age(X, AgeX), AgeX > 25,
...
% Потенциальные наследники: живые дети и внуки
potential_heir(Person, Heir) :-
    parent_of(Person, Heir),
    \+ death(_, Heir).

potential_heir(Person, Heir) :-
    parent_of(Person, Child),
    parent_of(Child, Heir),
    \+ death(_, Heir).

% Семья с максимальным числом детей
largest_family(Person, Count) :-
    setof(Child, parent_of(Person, Child), Children),
    length(Children, Count),
    \+ (setof(C, parent_of(_, C), OtherChildren), length(OtherChildren, OtherCount), OtherCount > Count).

% Овдовевшие люди
widowed(Person) :-
    (marriage(Person, Spouse, _); marriage(Spouse, Person, _)),
    death(_, Spouse),
    \+ death(_, Person),
    \+ divorce(Person, Spouse, _),
    \+ divorce(Spouse, Person, _).

% Кандидаты в крестные: старше 18, живы
godparent_candidate(Person) :-
    age(Person, Age),
    Age >= 18,
    \+ death(_, Person).

% Пары, которые отметят золотую свадьбу (50 лет) в ближайшие N лет
golden_wedding_soon(X, Y, Years) :-
    marriage(X, Y, MarriageYear),
    current_year(CurrentYear),
    \+ divorce(X, Y, _),
    \+ death(_, X),
    \+ death(_, Y),
    Duration is CurrentYear - MarriageYear,
    Remaining is 50 - Duration,
    Remaining > 0, Remaining =< Years.

% Одинокие родители: есть дети, но не в браке
single_parent(Person) :-
    parent_of(Person, _),
    \+ current_married(Person, _),
    \+ death(_, Person).

% Живые пенсионеры (старше 60 лет)
living_pensioner(Person, Age) :-
    age(Person, Age),
    Age >= 60,
    \+ death(_, Person).